...
import '../models/core/core_response_error.dart';
...
class CoreService extends get_x.GetxService {
  ...
  Future<void> performRequest({
    ...
    dynamic Function(CoreResponseError error)? onError,
  }) async {
    ...
    try {
      ...
      if (e.response?.statusCode == CoreHttpCode.TOKEN_EXPIRED) {
        ...
        if (refreshSuccess) {
          ...
          try {
            responseNext = await _requestApi(
              url,
              method,
              query,
              body,
          ...
          onError?.call(
            CoreResponseError(
              errorCode: CoreHttpCode.TOKEN_INVALID,
  ...
  void _handleDioError(
      DioError e, dynamic Function(CoreResponseError error)? onError, String url) async {
    switch (e.type) {
      case DioErrorType.connectTimeout:
      case DioErrorType.sendTimeout:
      case DioErrorType.receiveTimeout:
        LoggerService.log.e('Timeout error!');
        final error = CoreResponseError(
      ...
      case DioErrorType.response:
        ...
        try {
          error = CoreResponseError(
            ...
            message: '${e.response?.data[CoreRemoteConstants.errorMessageFieldKey]}',
          );
  ...
  Future<bool> requestRefreshToken() async {
    try {
      final refreshToken = CoreStorageManager().getRefreshToken();
      /// set up refresh token in header
      setUpHeader({
        CoreRemoteConstants.authorizationHeaderKey: 'Bearer $refreshToken',
      });

      LoggerService.log.i('Header: ${_dio.options.headers}');
      LoggerService.log.i('post -> ${_dio.options.baseUrl}'
          '${CoreRemoteConstants.refreshTokenEndPoint} '
          '\nrefresh-token: $refreshToken');

      Response? response = await _dio.post(CoreRemoteConstants.refreshTokenEndPoint);
      ...
      if ((response.statusCode ?? 0) >= 200 && (response.statusCode ?? 0) <= 299) {
        ...
        CoreStorageManager().setAccessToken(newAccessToken);
        CoreStorageManager().setRefreshToken(newRefreshToken);

        ///Set up access token in header
        setUpHeader({
          CoreRemoteConstants.authorizationHeaderKey: 'Bearer $newAccessToken',
        });
